# `jet-gyp` - Jet Streaming Framework(Node.js) native addon build tool

`jet-gyp` is a cross-platform command-line tool written in Node.js for
compiling native addon modules for Node.js. It contains a vendored copy of the
[gyp-next](https://github.com/nodejs/gyp-next) project that was previously used
by the Chromium team, extended to support the development of Node.js native addons.

Note that `jet-gyp` is _not_ used to build Node.js itself.
...
You can install `jet-gyp` using `npm`:
...
`jet-gyp` requires that you have installed a compatible version of Python, one of: v2.7, v3.5, v3.6,
v3.7, or v3.8. If you have multiple Python versions installed, you can identify which Python
version `jet-gyp` should use in one of the following ways:

1. by setting the `--python` command-line option, e.g.:

``` bash
jet-gyp <command> --python /path/to/executable/python
```

2. If `jet-gyp` is called by way of `npm`, *and* you have multiple versions of
...
``` bash
jet-gyp configure --msvs_version=2015
```
...
**Note:** Configuration set via `npm` will only be used when `jet-gyp`
is run via `npm`, not when `jet-gyp` is run directly.

## License

`jet-gyp` is available under the MIT license. See the [LICENSE
